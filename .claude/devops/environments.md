...
- 2026-06-07 — DMI-34: rewrote `build-push.yml` to match documented deploy mechanic: trigger on `dev` push + semver tags, dual-tag images (sha-<short>+dev / <tag>+latest), deploy job SCPs compose + docker compose pull/up, `workflow_dispatch` for manual re-deploy, correct secret names (DEPLOY_SSH_KEY, GHCR_TOKEN) and variable names (DEPLOY_HOST, DEPLOY_USER, DEPLOY_PATH).
